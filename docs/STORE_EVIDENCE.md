...
Store files under `release/` are intentionally ignored generated/manual evidence. They are not trusted merely because they exist. Every accepted graphic, metadata file, screenshot, and release summary must be tied to the exact clean canonical `origin/main` commit being prepared.
...
## 4. Author, finalize, and verify metadata

Author exactly these UTF-8 files under `release/google-play/metadata/en-US/`:
...
Before approval, finalize their evidence manifest:
...
5. candidate-bound store metadata.
...
- metadata text change: metadata manifest;
...
- store policy or product claims change: manual review and, where needed, metadata/screenshots.
