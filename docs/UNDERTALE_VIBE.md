...
- Partial: mercy hearts, pacifist rewards, and friendship persistence exist, and the run can now resolve into explicit `Kind`, `Merciful`, and `Peaceful` route tiers that carry into rest and Garden, with named peaceful-biome payoff plus persisted route-world signs instead of only generic route wording.
...
- TODO: full mercy reward language, stronger spare visibility, and deeper route-like world-state payoff beyond the current route-world signs.
