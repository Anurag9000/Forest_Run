 ...
 *
 * Callers must reject a frame with [acceptsDelta] or [accepts] before using the
 * bounding functions. A positive finite delta is capped to one 20 Hz recovery
 * step so a resumed or stalled frame cannot fast-forward simulation state.
 * Scroll speed is constrained to the canonical gameplay ceiling.
 */
internal object FrameInputAdmission {
    const val MAX_DELTA_SECONDS = 0.05f

    fun acceptsDelta(deltaSeconds: Float): Boolean =
        deltaSeconds.isFinite() && deltaSeconds > 0f

    fun accepts(deltaSeconds: Float, scrollSpeed: Float): Boolean =
        acceptsDelta(deltaSeconds) &&
            scrollSpeed.isFinite() &&
            scrollSpeed >= 0f

    fun boundedDeltaSeconds(deltaSeconds: Float): Float {
        require(acceptsDelta(deltaSeconds)) {
